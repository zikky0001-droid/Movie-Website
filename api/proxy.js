// proxy.js - Keep browser check, but allow downloads with 'name' parameter
...
export default async function handler(request) {
  ...
  // ✅ If it's a download request (has 'name' parameter), bypass the browser check
  // This allows Termux/curl downloads to work while keeping security for other requests
  const isDownloadRequest = Boolean(name);
  
  if (!isDownloadRequest) {
    // Only run browser check for non-download requests (streaming)
    if (!looksLikeFirstPartyBrowser(request)) {
